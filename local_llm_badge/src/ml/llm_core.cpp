...
static bool malloc_run_state(RunState* s, Config* p) {
    ...
    Serial.printf("RunState: need %dKB, free %dKB, kv_dim=%d, hidden=%d\n",
                  total_needed / 1024, ESP.getFreePsram() / 1024,
                  kv_dim, p->hidden_dim);
    ...
    // Debug: check which allocation failed
    if (!s->x) Serial.println("  x alloc failed");
    if (!s->xb) Serial.println("  xb alloc failed");
    if (!s->xb2) Serial.println("  xb2 alloc failed");
    if (!s->hb) Serial.println("  hb alloc failed");
    if (!s->hb2) Serial.println("  hb2 alloc failed");
    if (!s->q) Serial.println("  q alloc failed");
    if (!s->att) Serial.println("  att alloc failed");
    if (!s->logits) Serial.println("  logits alloc failed");
    if (!s->key_cache) Serial.println("  key_cache alloc failed");
    if (!s->value_cache) Serial.println("  value_cache alloc failed");
    if (!s->xq.q) Serial.println("  xq.q alloc failed");
    if (!s->xq.s) Serial.println("  xq.s alloc failed");
    if (!s->hq.q) Serial.println("  hq.q alloc failed");
    if (!s->hq.s) Serial.println("  hq.s alloc failed");

    if (!s->x || !s->xb || !s->xb2 || !s->hb || !s->hb2 || !s->q ||
        !s->att || !s->logits || !s->key_cache || !s->value_cache ||
        !s->xq.q || !s->xq.s || !s->hq.q || !s->hq.s) {
        Serial.printf("RunState: PSRAM after failed allocs: %dKB\n", ESP.getFreePsram() / 1024);
...
bool build_transformer_q8(Transformer* t, const char* checkpoint_path, int max_seq_len) {
    ...
    Serial.printf("LLM: PSRAM before raw_data: %dKB, need %dKB\n",
                  ESP.getFreePsram() / 1024, t->file_size / 1024);
    ...
    Serial.printf("LLM: PSRAM after embeddings: %dKB\n", ESP.getFreePsram() / 1024);
